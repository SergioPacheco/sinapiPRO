...
export interface MeasurementMemoLine {
  description: string;
  formula: string;
  value: number;
}
...
@Component({
  selector: 'app-measurement-memo-dialog',
  template: `
...
export class MeasurementMemoDialogComponent {
  private readonly dialogRef = inject(MatDialogRef<MeasurementMemoDialogComponent>);
  private readonly data = inject<MeasurementMemoLine | null>(MAT_DIALOG_DATA, { optional: true });
  line: MeasurementMemoLine = this.data ? { ...this.data } : { description: '', formula: '', value: 0 };
