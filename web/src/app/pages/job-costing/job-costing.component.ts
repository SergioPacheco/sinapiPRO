  ...
  template: `
    ...
    <p-table [value]="codes()" [loading]="loading()" styleClass="p-datatable-sm" [paginator]="true" [rows]="15">
      ...
      <ng-template pTemplate="body" let-r>
        <tr>
          <td class="font-mono">{{ r.code }}</td><td>{{ r.name }}</td>
          ...
          <td class="text-right" [style.color]="r.availableBalance >= 0 ? 'var(--green-400)' : 'var(--red-400)'">{{ r.availableBalance | number:'1.2-2' }}</td>
        </tr>
      </ng-template>
...
export class JobCostingComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get<any>(`/projects/${id}/cost-codes/budget-summary`).subscribe(s => {
      this.cards.set([
        { label: 'Orçado', value: s.totalBudgeted || 0 },
        { label: 'Comprometido', value: s.totalCommitted || 0 },
        { label: 'Realizado', value: s.totalActual || 0 },
        { label: 'Variação', value: s.totalVariance || 0 },
      ]);
    });
    this.http.get<any>(`/projects/${id}/cost-codes/summary`).subscribe({
