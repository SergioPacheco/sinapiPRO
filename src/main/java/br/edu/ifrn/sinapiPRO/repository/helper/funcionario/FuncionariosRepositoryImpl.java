...
public class FuncionariosRepositoryImpl implements FuncionariosRepositoryQueries {
	@PersistenceContext
	private EntityManager manager;
	private final PaginacaoUtil paginacaoUtil;

	public FuncionariosRepositoryImpl(PaginacaoUtil paginacaoUtil) {
	...
	public Page<Funcionario> filtrar(FuncionarioFilter f, Pageable p) {
		Criteria c = manager.unwrap(Session.class).createCriteria(Funcionario.class); paginacaoUtil.preparar(c, p);
		if (f != null && !StringUtils.isEmpty(f.getNome())) c.add(Restrictions.ilike("nome", f.getNome(), MatchMode.ANYWHERE));
		Criteria ct = manager.unwrap(Session.class).createCriteria(Funcionario.class);
