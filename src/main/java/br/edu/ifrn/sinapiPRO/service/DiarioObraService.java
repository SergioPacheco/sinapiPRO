...
public class DiarioObraService {
	...
	@Transactional
	public DiarioObra salvar(DiarioObra d) {
	...
	public void excluir(Long c) {
		try {
			...
			throw new ImpossivelExcluirEntidadeException("Impossível apagar o diário.");
		}
	}

	@Transactional(readOnly = true)
	public List<DiarioObra> findByObra(Long codigoObra) {
		return repository.findByObraCodigoOrderByDataDesc(codigoObra);
	}

	@Transactional(readOnly = true)
	public DiarioObra buscarComItens(Long codigo) {
		return repository.findById(codigo).orElseThrow(() -> new RuntimeException("Diário não encontrado"));
	}
}
