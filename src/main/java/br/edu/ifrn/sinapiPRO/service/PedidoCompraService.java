...
public class PedidoCompraService {
	@Autowired
	private PedidosCompraRepository repository;
	@Transactional
	public PedidoCompra salvar(PedidoCompra p) {
	...
	public void excluir(Long codigo) {
		try {
			...
			throw new ImpossivelExcluirEntidadeException("Impossível apagar o pedido.");
		}
	}

	@Transactional(readOnly = true)
	public List<PedidoCompra> findByObra(Long codigoObra) {
		return repository.findByObraCodigoOrderByDataPedidoDesc(codigoObra);
	}

	@Transactional(readOnly = true)
	public PedidoCompra buscarComItens(Long codigo) {
		return repository.findById(codigo).orElseThrow(() -> new RuntimeException("Pedido não encontrado"));
	}
}
