...
import br.edu.ifrn.sinapiPRO.service.FrotaService;
...
public ModelAndView lista() {
	...
	mv.addObject("veiculos", service.findVeiculos());
	return mv;
}

@GetMapping("/novo") public ModelAndView novoVeiculo(Veiculo v) {
	return new ModelAndView("veiculo/FormVeiculo");
}
...
@GetMapping("/{codigo}") public ModelAndView editarVeiculo(@PathVariable Long codigo) {
	ModelAndView mv = novoVeiculo(service.getVeiculo(codigo));
	mv.addObject(service.getVeiculo(codigo));
	return mv;
}
