...
import br.edu.ifrn.sinapiPRO.controller.page.PageWrapper; import br.edu.ifrn.sinapiPRO.model.Funcionario;
import br.edu.ifrn.sinapiPRO.repository.filter.FuncionarioFilter;
import br.edu.ifrn.sinapiPRO.service.*; import br.edu.ifrn.sinapiPRO.service.exception.*;
@Controller @RequestMapping("/funcionarios")
public class FuncionariosController {
	@Autowired private CadastroFuncionarioService service;
	@Autowired private CadastroCargoService cargoService;
	@Autowired private CadastroFuncaoService funcaoService;
	@Autowired private CadastroDepartamentoService departamentoService;
	@GetMapping("/novo") public ModelAndView novo(Funcionario f) {
		...
		return mv; }
	@PostMapping({"/novo","{\\d+}"}) public ModelAndView cadastrar(@Valid Funcionario f, BindingResult r, RedirectAttributes a) {
	...
	@GetMapping public ModelAndView pesquisar(FuncionarioFilter f, BindingResult r, @PageableDefault(size=25) Pageable p, HttpServletRequest req) {
