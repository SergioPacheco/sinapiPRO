...
import br.edu.ifrn.sinapiPRO.model.Requisicao; import br.edu.ifrn.sinapiPRO.repository.ObrasRepository;
import br.edu.ifrn.sinapiPRO.repository.InsumosRepository;
import br.edu.ifrn.sinapiPRO.service.RequisicaoService; import br.edu.ifrn.sinapiPRO.service.exception.*;
@Controller @RequestMapping("/requisicoes")
public class RequisicoesController {
	@Autowired private RequisicaoService service;
	...
	@GetMapping public ModelAndView lista(@RequestParam(required=false) Long codigoObra) {
		...
		return mv; }
	@GetMapping("/novo") public ModelAndView novo(Requisicao r) { return form(r); }
	@GetMapping("/{codigo}") public ModelAndView editar(@PathVariable Long codigo) { return form(service.buscarComItens(codigo)); }
	@PostMapping({"/novo","/{codigo}"}) public ModelAndView salvar(@Valid Requisicao r, BindingResult br, RedirectAttributes a) {
