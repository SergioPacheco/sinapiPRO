...
public class DepartamentosController {
	@Autowired
	private CadastroDepartamentoService service;
	
	@GetMapping("/novo")
	public ModelAndView novo(Departamento e) {
		return new ModelAndView("departamento/CadastroDepartamento");
	}

	
	@PostMapping({"/novo", "/{codigo}"})
	public ModelAndView cadastrar(@Valid Departamento e, BindingResult r, RedirectAttributes a) {
	...
	@GetMapping
	public ModelAndView pesquisar(DepartamentoFilter f, BindingResult r, @PageableDefault(size=25) Pageable p, HttpServletRequest req) {
