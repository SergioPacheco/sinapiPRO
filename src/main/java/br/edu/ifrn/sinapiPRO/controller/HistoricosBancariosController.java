...
import br.edu.ifrn.sinapiPRO.model.HistoricoBancario; import br.edu.ifrn.sinapiPRO.service.HistoricoBancarioService;
import br.edu.ifrn.sinapiPRO.service.exception.*;
@Controller @RequestMapping("/historicosBancarios")
public class HistoricosBancariosController {
	@Autowired private HistoricoBancarioService service;
	@GetMapping public ModelAndView lista() { ModelAndView mv = new ModelAndView("historicobancario/ListaHistoricosBancarios"); mv.addObject("historicos", service.findAll()); return mv; }
	@GetMapping("/novo") public ModelAndView novo(HistoricoBancario h) { return new ModelAndView("historicobancario/CadastroHistoricoBancario"); }
	@PostMapping({"/novo","{\\d+}"}) public ModelAndView salvar(@Valid HistoricoBancario h, BindingResult r, RedirectAttributes a) {
