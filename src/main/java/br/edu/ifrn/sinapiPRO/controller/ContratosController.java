...
import br.edu.ifrn.sinapiPRO.model.Contrato; import br.edu.ifrn.sinapiPRO.repository.ObrasRepository;
import br.edu.ifrn.sinapiPRO.repository.ClientesRepository;
import br.edu.ifrn.sinapiPRO.service.ContratoService; import br.edu.ifrn.sinapiPRO.service.exception.*;
@Controller @RequestMapping("/contratos")
public class ContratosController {
	...
	@GetMapping public ModelAndView lista(@RequestParam(required=false) Long codigoObra) {
		...
		return mv; }
	@GetMapping("/novo") public ModelAndView novo(Contrato c) { return form(c); }
	@GetMapping("/{codigo}") public ModelAndView editar(@PathVariable Long codigo) { return form(service.buscarComItens(codigo)); }
	@PostMapping({"/novo","/{codigo}"}) public ModelAndView salvar(@Valid Contrato c, BindingResult r, RedirectAttributes a) {
