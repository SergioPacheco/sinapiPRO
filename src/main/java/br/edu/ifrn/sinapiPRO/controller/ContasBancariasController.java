...
import br.edu.ifrn.sinapiPRO.model.ContaBancaria; import br.edu.ifrn.sinapiPRO.service.ContaBancariaService;
import br.edu.ifrn.sinapiPRO.service.exception.*;
@Controller @RequestMapping("/contasBancarias")
public class ContasBancariasController {
	@Autowired private ContaBancariaService service;
	@GetMapping public ModelAndView lista() { ModelAndView mv = new ModelAndView("contabancaria/ListaContasBancarias"); mv.addObject("contas", service.findAll()); return mv; }
	@GetMapping("/novo") public ModelAndView novo(ContaBancaria c) { return new ModelAndView("contabancaria/CadastroContaBancaria"); }
	@PostMapping({"/novo","{\\d+}"}) public ModelAndView salvar(@Valid ContaBancaria c, BindingResult r, RedirectAttributes a) {
