...
import br.edu.ifrn.sinapiPRO.model.PlanoContas; import br.edu.ifrn.sinapiPRO.service.PlanoContasService;
import br.edu.ifrn.sinapiPRO.service.exception.*;
@Controller @RequestMapping("/planoContas")
public class PlanoContasController {
	@Autowired private PlanoContasService service;
	@GetMapping public ModelAndView lista() { ModelAndView mv = new ModelAndView("planocontas/ListaPlanoContas"); mv.addObject("contas", service.findAll()); return mv; }
	@GetMapping("/novo") public ModelAndView novo(PlanoContas p) { ModelAndView mv = new ModelAndView("planocontas/CadastroPlanoContas"); mv.addObject("pais", service.findAll()); return mv; }
	@PostMapping({"/novo","{\\d+}"}) public ModelAndView salvar(@Valid PlanoContas p, BindingResult r, RedirectAttributes a) {
