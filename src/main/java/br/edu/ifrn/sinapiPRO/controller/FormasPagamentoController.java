...
import br.edu.ifrn.sinapiPRO.controller.page.PageWrapper; import br.edu.ifrn.sinapiPRO.model.FormaPagamento;
import br.edu.ifrn.sinapiPRO.repository.filter.FormaPagamentoFilter; import br.edu.ifrn.sinapiPRO.service.CadastroFormaPagamentoService;
import br.edu.ifrn.sinapiPRO.service.exception.*;
@Controller @RequestMapping("/formasPagamento")
public class FormasPagamentoController {
	@Autowired private CadastroFormaPagamentoService service;
	@GetMapping("/novo") public ModelAndView novo(FormaPagamento fp) { return new ModelAndView("formapagamento/CadastroFormaPagamento"); }
	@PostMapping({"/novo","{\\d+}"}) public ModelAndView cadastrar(@Valid FormaPagamento fp, BindingResult r, RedirectAttributes a) {
		if (r.hasErrors()) return novo(fp);
		try { service.salvar(fp); } catch (JaCadastradoException e) { r.rejectValue("nome", e.getMessage(), e.getMessage()); return novo(fp); }
		a.addFlashAttribute("mensagem", "Forma de pagamento salva!"); return new ModelAndView("redirect:/formasPagamento/novo"); }
	@GetMapping public ModelAndView pesquisar(FormaPagamentoFilter f, BindingResult r, @PageableDefault(size=25) Pageable p, HttpServletRequest req) {
